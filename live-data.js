    ...
    openf1("/sessions", { year: new Date().getFullYear() }).then(function (sessions) {
      ...
      function fetchRaceDetail(s) {
        return Promise.all([
          ...
          var top1 = results[0];
          // ⚠️ OpenF1이 레이스 직후엔 session_result를 아직 채우기 전이라
          // 빈 배열을 줄 때가 있어요. 이때 예전 코드는 home:"F1"/away:"Race"
          // 같은 뜻 없는 가짜 매치를 "undefined-undefined" 스코어로 만들어서
          // 보여줬는데, 이것도 가짜 정보나 마찬가지라 아예 화면에 안 보이게
          // null을 반환해서 걸러내요(위쪽 결과 없는 세션 항목으로만 잡혀요).
          if (!top1) return null;
          ...
          return {
            ...
            league: "F1",
            venue: s.circuit_short_name || s.location || "",
            home: nameFor(top1.driver_number),
            away: top2 ? nameFor(top2.driver_number) : "",
            status: "finished",
            homeScore: top1.position,
